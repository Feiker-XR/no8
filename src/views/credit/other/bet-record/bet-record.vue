  ...
  <scroller :height="'-46px'" class="betRecord" lock-x scrollbar-y @on-scroll-bottom="searchMore" :scroll-bottom-offst="200">
    <div>
      <x-table :cell-bordered="false" style="background-color:#fff">
        ...
        <tbody>
        <tr v-for="(item,index) in dataList" @click="handleOpenDialog(item)">
          <td>{{item.lotteryGameName}}</td>
          <td>{{item.betContent}}</td>
          <td>{{item.odds}}</td>
          <td>{{item.betAmount}}元</td>
    ...
    <div v-transfer-dom @touchmove.prevent>
      <x-dialog v-model="dialogVisable">
        <group label-align="left">
          ...
          <cell-box class="group-cell" title="订单号" :border-intent="false" :arrow-direction="'up'">
           <span>订单号</span>
            <p class="orderID">
              {{form.billNo}}
            </p>
            <button @click="copyWords(form.billNo)">复制</button>
          </cell-box>
